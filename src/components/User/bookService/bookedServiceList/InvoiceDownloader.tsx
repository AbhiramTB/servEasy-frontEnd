...
import { BookingData } from '../BookedOfflineServicePage'
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24, fontSize: 14 }}>
          <div style={{ flex: 1, paddingRight: 16 }}>
            <h3 style={{ marginBottom: 8, borderBottom: '1px solid #e2e8f0', paddingBottom: 4, color: '#2563eb' }}>Customer Details</h3>
            <p style={{ margin: '4px 0' }}><strong>Name:</strong> {address.name}</p>
            <p style={{ margin: '4px 0' }}>
              <strong>Address:</strong> {address.houseName}, {address.state} - {address.pincode}
            </p>
            <p style={{ margin: '4px 0' }}><strong>Phone:</strong> {address.phone}</p>
          </div>

          <div style={{ flex: 1, paddingLeft: 16 }}>
            <h3 style={{ marginBottom: 8, borderBottom: '1px solid #e2e8f0', paddingBottom: 4, color: '#2563eb' }}>Service Provider Details</h3>
            <p style={{ margin: '4px 0' }}><strong>Name:</strong> {serviceProvider.serviceProviderName}</p>
            <p style={{ margin: '4px 0' }}><strong>Email:</strong> {serviceProvider.serviceProviderEmail}</p>
            <p style={{ margin: '4px 0' }}><strong>Phone:</strong> {serviceProvider.serviceProviderPhone}</p>
          </div>
        </div>
        ...
        <div style={{ marginBottom: 24, fontSize: 14 }}>
          ...
          <p><strong>Booking Date:</strong> {new Date(bookedTime).toLocaleDateString()}</p>
        ...
        >
          ...
          <tbody>
            <tr>
              ...
              <td style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'right' }}>{payment?.serviceCost?.toFixed(2) ?? '0.00'}</td>
            </tr>
            <tr>
              <td style={{ padding: '8px', border: '1px solid #ddd' }}>Material Cost</td>
              <td style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'right' }}>{payment?.metaialCost?.toFixed(2) ?? '0.00'}</td>
            </tr>
            <tr>
              <td style={{ padding: '8px', border: '1px solid #ddd' }}>Travel Cost</td>
              <td style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'right' }}>{payment?.travelCost?.toFixed(2) ?? '0.00'}</td>
            </tr>
            <tr>
              <td style={{ padding: '8px', border: '1px solid #ddd' }}>Inspection Cost</td>
              <td style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'right' }}>{payment?.inspectionCost?.toFixed(2) ?? '0.00'}</td>
            </tr>
            <tr>
              <td style={{ padding: '8px', border: '1px solid #ddd' }}>Convenience Fee</td>
              <td style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'right' }}>{payment?.convenienceFee?.toFixed(2) ?? '0.00'}</td>
            </tr>
            <tr style={{ fontWeight: 'bold', backgroundColor: '#f3f4f6' }}>
              <td style={{ padding: '8px', border: '1px solid #ddd' }}>Total Paid</td>
              <td style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'right' }}>{payment?.total.toFixed(2)}</td>
            </tr>
          </tbody>
        </table>
