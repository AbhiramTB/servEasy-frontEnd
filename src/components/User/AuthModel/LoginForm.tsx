import { FormEvent, useState, ChangeEvent, SetStateAction, Dispatch } from "react";
...
const LoginForm = ({ setForgotPassword, setError }: Props) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setLoading(true);
    await handleAuth(formData, true, setLoading, setError, navigate, useEmail);
  };
    ...
    <form onSubmit={handleSubmit} className="mt-6 space-y-6">
      ...
      <div className="form-control">
        <div className="flex items-center justify-between mb-2">
          <label className="font-medium label-text">Password</label>
          <button 
            type="button" 
            className="font-medium transition-colors duration-200 link link-primary" 
        ...
        <div className="relative group">
          <input
            ...
            className="w-full pr-12 text-lg transition-all duration-300 input input-bordered input-primary input-lg bg-base-100/80 backdrop-blur-sm focus:input-primary focus:shadow-lg"
